...
class Enzyme(Drawable):
    ...
    def bounceOff(self, other, entityList = None):
        ...
        # if metabolite
            ...
            if other.name in self.reactants:
                ...
                self.loaded[other.name] += 1
                #todo multiple reactant [with corresponding multiple sources]
                #if it has at least one of every metabolite that it needs
                if min([self.loaded[x] for x in self.loaded]) > 0:
                    #decrement all reactants by one
                    for sp in self.loaded:
                        self.loaded[sp] -= 1
                    #bounce other, and make all reactants
                    tmp = other
    ...
    #                        deltax = -3
    #                        deltay = 3
                            r = Metabolite(name=self.products[i],x=other.x + deltax * 3,y=other.y + 3 * deltay,xvel=other.xvel + deltax,yvel=other.yvel + deltay)
                            entityList.append(r)
                else:
                    self.explode(other)
            else:
